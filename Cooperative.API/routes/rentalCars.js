...
const {rentalModel, validateRental } = require("../models/rentalModel");
...
router.get("/", async(req,res) => {
  ...
  try{
    ...
    let data = await rentalModel.find(objFind)
    .limit(perPage)
    .skip(page * perPage)
    .sort({[sort]:reverse})
    res.json(data);
...
router.get("/allCars", async(req,res) => {
  try{
    let data = await rentalModel.find({})
    res.json(data);
  }
  catch(err){
    console.log(err)
...
router.get("/single/:id", async(req,res) => {
  ...
  try{
    ...
    let data = await rentalModel.findOne({_id:id})
    res.json(data);
  }
  catch(err){
    console.log(err)
...
router.post("/",authAdmin , async(req,res) => {
  let validBody = validateRental(req.body);
  if (validBody.error) {
    return res.status(400).json(validBody.error.details);
  }
  try{
    let product = new rentalModel(req.body);
...
router.put("/:idEdit",authAdmin , async(req,res) => {
  ...
  try{
    ...
    let data = await rentalModel.updateOne({_id:idEdit},req.body);
...
router.delete("/:idDel",authAdmin , async(req,res) => {
  try{
    ...
    let data = await rentalModel.deleteOne({_id:idDel});
    res.status(200).json(data);
  }
  catch(err){
    console.log(err);
...
const genShortId = async() => {
  ...
  while(flag){
    rnd = random(0,999999)
    try{
      let data = await rentalModel.findOne({short_id:rnd})
      if(!data){
        flag = false;
      }
    }
