...
const { validateCategory, CategoryModel } = require("../models/categorySaleModel");
...
router.delete("/:idDelete", authAdmin , async(req,res) => {
  try{
    ...
    let dataSale = await saleModel.deleteMany({cat_short_id:catShortId});
    let data = await CategoryModel.deleteOne({_id:idDelete});
    res.json(data,dataSale);
  }
  catch(err){
    console.log(err);
    return res.status(500).json(err);
